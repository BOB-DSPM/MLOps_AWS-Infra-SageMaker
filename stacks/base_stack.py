...
class BaseStack(Stack):
    def __init__(
        ...
        **kwargs
        ...
        enable_feature_group = bool(self.node.try_get_context("enable_feature_group"))
        use_existing_feature_group = bool(self.node.try_get_context("use_existing_feature_group"))
        ...
        if enable_feature_group and not use_existing_feature_group:
            ...
            self.feature_group_name = feature_group_name  # 운영 Feature Group 이름을 그대로 사용

        # 사용자 상호작용 데이터용 Feature Group도 필요 시에만 생성
        if enable_feature_group and not use_existing_feature_group:
