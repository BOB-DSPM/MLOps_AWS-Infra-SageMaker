                    ...
                    "phases": {
                        ...
                        "build": {
                            "commands": [
                                ...
                                "resp=sm.list_model_packages(ModelPackageGroupName=GROUP, SortBy='CreationTime', SortOrder='Descending', ModelApprovalStatus='Approved')\n"
                                "if not resp['ModelPackageSummaryList']:\n"
                                "  print('No approved model package found in group:', GROUP)\n"
                                "  # Try to get any model package and approve it\n"
                                ...
                                "  print('Approving model package:', mp)\n"
                                "  sm.update_model_package(ModelPackageArn=mp, ModelApprovalStatus='Approved', ApprovalDescription='Auto-approved by CodePipeline')\n"
                                "else:\n"
                                "  mp=resp['ModelPackageSummaryList'][0]['ModelPackageArn']\n"
                                "  print('Using approved model package:', mp)\n"
                                "# Verify model package has valid model data\n"
                                "try:\n"
                                "  mp_details = sm.describe_model_package(ModelPackageName=mp)\n"
                                "  containers = mp_details['InferenceSpecification']['Containers']\n"
                                "  model_data_found = False\n"
                                "  for container in containers:\n"
                                "    if 'ModelDataUrl' in container:\n"
                                "      model_url = container['ModelDataUrl']\n"
                                "      print(f'Checking model data at: {model_url}')\n"
                                "      if model_url.startswith('s3://'):\n"
                                "        bucket, key = model_url.replace('s3://','').split('/',1)\n"
                                "        try:\n"
                                "          s3.head_object(Bucket=bucket, Key=key)\n"
                                "          print(f'Model data verified at: {model_url}')\n"
                                "          model_data_found = True\n"
                                "          break\n"
                                "        except Exception as e:\n"
                                "          print(f'WARNING: Model data not found at {model_url}: {e}')\n"
                                "          print('Trying to find latest training job model...')\n"
                                "          # Find the latest completed training job\n"
                                "          training_jobs = sm.list_training_jobs(StatusEquals='Completed', SortBy='CreationTime', SortOrder='Descending', MaxResults=10)\n"
                                ...
                                "              latest_model_url = job_details['ModelArtifacts']['S3ModelArtifacts']\n"
                                "              print(f'Found latest model at: {latest_model_url}')\n"
                                "              # Update model package with correct URL\n"
                                "              container['ModelDataUrl'] = latest_model_url\n"
                                "              bucket, key = latest_model_url.replace('s3://','').split('/',1)\n"
                                "              try:\n"
                                "                s3.head_object(Bucket=bucket, Key=key)\n"
                                "                print(f'Latest model data verified at: {latest_model_url}')\n"
                                "                model_data_found = True\n"
                                "                break\n"
                                "              except Exception:\n"
                                "                continue\n"
                                "          if model_data_found:\n"
                                "            break\n"
                                "  if not model_data_found:\n"
                                "    print('WARNING: No valid model data found, proceeding with model package as-is')\n"
                                "except Exception as e:\n"
                                "  print(f'WARNING: Failed to verify model package: {e}')\n"
